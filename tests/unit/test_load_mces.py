...
class TestLoadMCES:
    ...
    def test_find_file(self, tmp_path):
        ...
        file1 = test_dir / "prefix_chunk0.npy"
        file2 = test_dir / "prefix_chunk1.npy"
        file3 = test_dir / "other_chunk0.npy"
        ...
        assert len(result) == 2
        assert any("prefix_chunk0.npy" in path for path in result)
        assert any("prefix_chunk1.npy" in path for path in result)
        assert not any("other_chunk0.npy" in path for path in result)
    ...
    def test_find_file_nested_directories(self, tmp_path):
        ...
        file1 = test_dir / "prefix_chunk0.npy"
        file2 = subdir / "prefix_chunk1.npy"
    ...
    def test_load_raw_data_with_partitions_limit(self, tmp_path):
        ...
        np.save(test_dir / "prefix_chunk0.npy", data1)
        np.save(test_dir / "prefix_chunk1.npy", data2)
        np.save(test_dir / "prefix_chunk2.npy", data3)
    ...
    def test_load_mces_20_data(self, tmp_path):
        ...
        np.save(folder0 / "mces.npy", data0)
        np.save(folder1 / "mces.npy", data1)
        np.save(folder2 / "mces.npy", data2)
