...
with the official MassSpecGym train/val/test split (train re-split by Murcko
scaffold into train/val, official val kept separate as val_official, official
test kept as test) — same split structure as the original preprocessing_msg_official
run, not Gaetan's split.
...
No separate exact-recompute pass for the [10,20] band — the max(lb, hdf5) value
is used as-is everywhere.
"""
...
OUT_DIR = Path(
    "/sofia/projects/2026_053/simba_project/data/massspecgym/preprocessing_official_split_max_lb_hdf5"
)
...
def main():
    ...
    # 1. Read MGF, using MassSpecGym's own official fold labels
    print("Reading MGF...")
    ...
    # 4. Load HDF5 SMILES index and mces array into RAM (~2.4 GB)
    ...
    # 6. Murcko scaffold split of official train (SEED=42, VAL_FRAC=0.10)
    print(
