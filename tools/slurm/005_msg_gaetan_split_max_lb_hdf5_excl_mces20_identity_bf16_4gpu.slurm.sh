#!/bin/bash
#SBATCH -J simba_train_005_gaetan_split
#SBATCH -p zen4_h200
...
#SBATCH -o /sofia/projects/2026_053/simba_project/experiments/training/005_msg_gaetan_split_max_lb_hdf5_excl_mces20_identity_bf16_4gpu/%x_%j.out
#SBATCH -e /sofia/projects/2026_053/simba_project/experiments/training/005_msg_gaetan_split_max_lb_hdf5_excl_mces20_identity_bf16_4gpu/%x_%j.err

# Experiment 005: same hyperparameters as experiment 004 (MCES==20 excluded,
# identity pairs added, bf16/DDP setup), but trained on
# data/massspecgym/preprocessing_gaetan_split_max_lb_hdf5 — Gaetan's own
# train/val/test split assignment (tools/prepare_msg_gaetan_split_max_lb_hdf5.py),
# with MCES = max(lb_matrix, HDF5) using the corrected HDF5 index formula, and
# no separate exact-recompute pass for the [10,20] band.
#
# This preprocessing directory has only ONE val set (Gaetan's), not a
# scaffold/official pair like experiments 001-004 — there is nothing to adapt
# here for that, though: prepare_data()/setup_callbacks() already derive
# val_names from whether a val_official split is present
# (simba/commands/train.py: `val_names = ["scaffold", "official"] if
# dataset_val_official is not None else ["val"]`), so a single-val mapping.pkl
# is handled automatically — same CLI overrides as 004 below, no new flags.
#
...
echo "===== SIMBA Training 005: Gaetan split, max(lb_matrix, HDF5) fixed indexing, excluding MCES==20, +identity pairs · no metadata · 4x H200 DDP · global bs=16384 · bf16-mixed · lr=0.00028 ====="
...
PREPRO_DIR=/sofia/projects/2026_053/simba_project/data/massspecgym/preprocessing_gaetan_split_max_lb_hdf5
MGF=/sofia/projects/2026_053/simba_project/data/massspecgym/data/auxiliary/MassSpecGym.mgf
OUTPUT_DIR=/sofia/projects/2026_053/simba_project/experiments/training/005_msg_gaetan_split_max_lb_hdf5_excl_mces20_identity_bf16_4gpu
