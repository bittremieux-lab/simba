#!/bin/bash
#SBATCH -J simba_train_014_2_logloss_a5
#SBATCH -p zen4_h200
...
#SBATCH -o /sofia/projects/2026_053/simba_project/experiments/training/014_2_logloss_a5_1gpu/%x_%j.out
#SBATCH -e /sofia/projects/2026_053/simba_project/experiments/training/014_2_logloss_a5_1gpu/%x_%j.err
...
# model.tasks.mces.use_log_loss=true. a=5 keeps the DEFAULT/historical warp
# strength (log_loss_a's default) -- so this run's only variable, relative
# to 014_2 itself, is turning the log-warp ON at its existing default
# strength (implied pseudocount log(MCES+8)). The companion run
# 014_2_logloss_a40_1gpu.slurm.sh instead uses a=40 (pseudocount log(MCES+1),
# what Gaetan actually proposed) -- comparing these two plus the 014_2
# baseline (log-loss off entirely) isolates "does log-loss help at all" from
# "does the stronger compression specifically help".
#
...
echo "===== SIMBA Training 014_2 + log-loss (a=5, pseudocount=8) ====="
...
OUTPUT_DIR=/sofia/projects/2026_053/simba_project/experiments/training/014_2_logloss_a5_1gpu
