#!/bin/bash
#SBATCH -J simba_train_meta_noce_v2
#SBATCH -p one_day
...
#SBATCH --mem=180G
#SBATCH -o /home/nkubrakov/simba/logs/simba_train_meta_noce_v2_%j.out
#SBATCH -e /home/nkubrakov/simba/logs/simba_train_meta_noce_v2_%j.err

# clip-40 + metadata, CE fix NOT applied (loader reads only params["ce"], so MSG
# spectra get CE=0). Control run to confirm MCES sampling bug fix restores 7636 dynamics.
# 8 epochs (~80k steps) for quick comparison.

set -uo pipefail

echo "===== SIMBA Training: MSG scaffold split — metadata, no CE fix (v2, bug fixed) ====="
...
OUTPUT_DIR=/mnt/data/nkubrakov/experiments_3_dataset/training/msg_scaffold_split_mces40_metadata_noce_v2
