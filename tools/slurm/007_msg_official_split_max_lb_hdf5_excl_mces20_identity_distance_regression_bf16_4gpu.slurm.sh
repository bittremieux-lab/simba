#!/bin/bash
#SBATCH -J simba_train_007_distance_regression
#SBATCH -p zen4_h200
...
#SBATCH -o /sofia/projects/2026_053/simba_project/experiments/training/007_msg_official_split_max_lb_hdf5_excl_mces20_identity_distance_regression_bf16_4gpu/%x_%j.out
#SBATCH -e /sofia/projects/2026_053/simba_project/experiments/training/007_msg_official_split_max_lb_hdf5_excl_mces20_identity_distance_regression_bf16_4gpu/%x_%j.err

# Experiment 007: identical to experiment 006 (official split, fixed MCES,
# MCES==20 excluded, identity pairs, bf16/DDP setup) except for one change —
# model.tasks.cosine_similarity.use_distance_regression=true, which swaps the
# cosine-similarity head for a Euclidean-distance-derived similarity
# (exp(-||e0-e1||)) and skips the log-warp loss transform. See
# simba/core/models/similarity_models.py: compute_from_embeddings/step.
...
echo "===== SIMBA Training 007: official split, fixed MCES, distance-regression head · no metadata · 4x H200 DDP · global bs=16384 · bf16-mixed · lr=0.00028 ====="
...
OUTPUT_DIR=/sofia/projects/2026_053/simba_project/experiments/training/007_msg_official_split_max_lb_hdf5_excl_mces20_identity_distance_regression_bf16_4gpu
...
srun uv run simba train \
  ...
  model.tasks.cosine_similarity.use_distance_regression=true
