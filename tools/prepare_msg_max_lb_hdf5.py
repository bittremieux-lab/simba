...
For each pair:
  - Gaetan lb_matrix: tight lower bound for MCES >= 10, underestimate for < 10
...
OUT_DIR = Path("/mnt/data/nkubrakov/massspecgym/preprocessing_msg_max_lb_hdf5")
...
def build_pairs(local_mol_indices, lb_indices, lb, hdf5_indices, hdf5_mces):
    """
...
def main():
    ...
    # 6. Murcko scaffold split of official train (identical to gaetan_official: SEED=42, VAL_FRAC=0.10)
    print(
    ...
    for fold, (df_fold, _si, lb_fold_idxs, hdf5_fold_idxs) in splits.items():
        ...
        pairs = build_pairs(
            local_indices, global_lb_idxs, lb, global_hdf5_idxs, hdf5_mces
        )
