...
ax.set_title("val_loss per trial")
ax.set_xticks(ids)
...
for idx, color in [(BASELINE_IDX, "#2196F3"), (BEST_IDX, "#4CAF50")]:
    ax.annotate(
        f"#{idx}\n{values[idx]:.3f}",
        xy=(idx, values[idx]),
        xytext=(0, 8 if values[idx] >= 0 else -24),
...
ax2.set_title("Optimisation path (running best)")
ax2.set_xticks(ids)
ax2.grid(alpha=0.3)
...
fig.colorbar(
    ScalarMappable(norm=norm, cmap=cmap), ax=axes[-1], label="val_loss", shrink=0.8
)
axes[0].legend(
    handles=[
...
fig.suptitle(
    "Parallel coordinates — hyperparameter landscape", fontsize=13, fontweight="bold"
)
...
for idx, color, label in [
    (
        BASELINE_IDX,
        "#2196F3",
        f"Baseline (#{BASELINE_IDX}, val={values[BASELINE_IDX]:.3f})",
    ),
...
ax.set_xticks(x_pos)
ax.set_xticklabels(
    ["lr (log)", "wd (log)", "d_model", "n_layers", "dropout", "val_loss"], fontsize=10
)
...
fig.colorbar(
    ScalarMappable(norm=norm, cmap=cmap), ax=ax, label="val_loss", shrink=0.8, pad=0.01
)
...
def parity_ax(ax, fig, true, pred, r, title, color):
    ...
    hb = ax.hexbin(
        true, pred, gridsize=40, cmap="Blues", mincnt=1, linewidths=0.2, norm=LogNorm()
    )
...
parity_ax(
    ...
    inf["ed_pred"],
    inf["ed_correlation"],
    f"Edit Distance (ED)  ·  Pearson r = {inf['ed_correlation']:.4f}",
    "#E91E63",
)
parity_ax(
    axes[1],
    ...
    inf["mces_true"],
    inf["mces_pred"],
    inf["mces_correlation"],
    f"MCES  ·  Pearson r = {inf['mces_correlation']:.4f}",
    "#FF9800",
)
...
for t in sorted(trials, key=lambda x: x["value"]):
    i = t["trial_number"]
    p = t["params"]
    note = ("◀ baseline" if i == BASELINE_IDX else "") + (
        "★ BEST" if i == BEST_IDX else ""
    )
    print(
        f"{i:>4}  {t['value']:>10.4f}  {p['optimizer.lr']:>12.2e}  {p['optimizer.weight_decay']:>12.2e}  "
