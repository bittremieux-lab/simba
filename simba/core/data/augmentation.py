...
class Augmentation:
    ...
    def augment(
        ...
        # precursor mass: perturb it to reduce the model's reliance on precursor
        # mass alone (Sebastian's original rationale). "legacy" = uniform noise
        # up to 1% of precursor m/z; "mist_cf" = MIST-CF/BUDDY-style Gaussian
        # noise scaled by instrument ppm tolerance (Wout/Gaetan's suggestion);
        # "none" = no precursor-mass perturbation at all.
    ...
    def resample_precursor_masses_mist_cf(sample, p_augmentation=0.2):
        """
        MIST-CF/BUDDY-style precursor m/z resampling (Wout/Gaetan's suggestion):
        replace each side's precursor mass with a Gaussian-perturbed value, std =
        instrument-specific ppm tolerance / 5. Meant to be applied on top of a
        theoretical (formula/SMILES-derived) base precursor mass -- replaces the
        uniform +/-1% noise in `add_false_precursor_masses_positives` above.
        Falls back to the "unknown" instrument tier (15 ppm) when a side's
        instrument type isn't present in `sample`.
        """
        if random.random() < p_augmentation:
            for suffix in ("_0", "_1"):
