...
## MAIN PARAMETERS
SUFIX="unique_compounds_mh"
REMOVE_REDUNDANT_SPECTRA=False
GOOD_QUALITY_FILTERING=False
USE_NEW_RANKING=False
...
if USE_TFS_SPECTRA:
    OUTPUT_PICKLE_FILE = os.path.join(OUTPUT_DIR, f"analog_discovery_simba_modified_cosine_results_tf_{SUFIX}.pkl")
else:
    OUTPUT_PICKLE_FILE = os.path.join(OUTPUT_DIR, f"analog_discovery_simba_modified_cosine_results_msnlib_{SUFIX}.pkl")
...
cfg.data.preprocessing.min_n_peaks=6
...
if USE_TFS_SPECTRA:
    ...
    #reference_file = "/data/simba_files_/tfs_ms2_merged_ref_auto.mgf"
else:
...
all_spectrums_reference = [s_original for s_original, s_processed in zip(all_spectrums_reference,all_spectrums_reference_processed) if len(s_processed.mz)>=cfg.data.preprocessing.min_n_peaks]

print(f'Basic filtering: {len(all_spectrums_reference)}')
### Filtering for good quality:

if GOOD_QUALITY_FILTERING:
    all_spectrums_reference_processed= [copy.deepcopy(s) for s in all_spectrums_reference]
    all_spectrums_reference_processed = [pp.preprocess_spectrum(
            s,
            fragment_tol_mass=10,
    ...
    all_spectrums_reference = [s_original for s_original, s_processed in zip(all_spectrums_reference,all_spectrums_reference_processed) if len(s_processed.mz)>=3]


    print(f'Extra filtering: {len(all_spectrums_reference)}')
    ...
    if PUT_METADATA_TO_ZEROS_REFERENCE:
        for j,s in enumerate(all_spectrums_reference):
          ...
          for j,s in enumerate(all_spectrums_reference):
            ...
            #print('')
            #print(f'Spec: {j}')
            #print(f'Collision energy value: {ce}')
            all_spectrums_reference[j].params['ce']=ce
            setattr(all_spectrums_reference[j], 'ce', ce)


            #print(f'Ionization method: {s.params["ionization_method"]}')
            all_spectrums_reference[j].params['ionization_method']=s.params['ionization_method']
            setattr(all_spectrums_reference[j], 'ionization_method', s.params['ionization_method'])

            #print(f'ion_activation: {s.params["ion_activation"]}')
            all_spectrums_reference[j].params['ion_activation']=s.params['ion_activation']
            setattr(all_spectrums_reference[j], 'ion_activation', s.params['ion_activation'])

            #print(f'adduct: {s.params["adduct"]}')
            all_spectrums_reference[j].params['adduct']=s.params['adduct']
            setattr(all_spectrums_reference[j], 'adduct', s.params['adduct'])

            #print(f'ionmode: {s.params["ionmode"].lower()}')
...
# FILTER BY ADDUCT:
if TARGET_ADDUCT is not None:
    all_spectrums_reference = [s for s in all_spectrums_reference if TARGET_ADDUCT in s.params['adduct'] ]
    all_spectrums_query = [s for s in all_spectrums_query if TARGET_ADDUCT in s.params['adduct']  ]
...
## Normalize reference
#for i,s in enumerate(all_spectrums_reference):
#    all_spectrums_reference[i].intensity = 100*np.array(s.intensity)/max(s.intensity)
...
def get_top_k_candidates(ranking, reference_spectra, k=10, unique_compounds=True):

    #select first ranking 100
    k_100=100
    top_indices = np.argsort(ranking, axis=1)[:, -k_100:][:, ::-1]
...
def score_simba_new_ranking(query_spectra, reference_spectra):
    if simba_model is None:
        raise RuntimeError(
            "SIMBA model was not initialized. Include 'simba' in "
            "SCORING_METHODS and rerun the notebook."
        )
    ...
    # The model outputs floating-point predictions, but MCES and edit
    # distance are discrete quantities.
    sim_ed_integer = np.rint(sim_ed).astype(int)
    sim_mces_integer = np.rint(sim_mces).astype(int)

    query_precursor_mz = np.asarray(
        [
    ...
    reference_precursor_mz = np.asarray(
        [
            spectrum.precursor_mz
    ...
    # Shape: (n_queries, n_references)
    precursor_mz_differences = np.abs(
        query_precursor_mz[:, np.newaxis]
        - reference_precursor_mz[np.newaxis, :]
    )

    ranking = AnalogDiscovery.compute_ranking_with_precursor_mz(
        similarities_mces=sim_mces_integer,
        similarities_ed=sim_ed_integer,
        precursor_mz_differences=precursor_mz_differences,
    )

    return {
        "ranking": ranking,
        "sim_ed": sim_ed_integer,
        "sim_mces": sim_mces_integer,
        "sim_ed_raw": sim_ed,
        "sim_mces_raw": sim_mces,
        "precursor_mz_difference": precursor_mz_differences,
    }
...
def run_scoring_method(method, query_spectra, reference_spectra):
    ...
    if method == "simba":
        if USE_NEW_RANKING:
            result =score_simba_new_ranking(query_spectra, reference_spectra)
        else:
    ...
    top_spectra, top_scores, top_indices = function(ranking, reference_spectra, k=TOP_K)
