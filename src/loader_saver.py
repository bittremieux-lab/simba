...
class LoaderSaver:
    ...
    def load_and_save_nist(self, file, num_samples=100, compute_classes=False, use_tqdm=True, config=None):
        ...
        for m in range(0,number_of_blocks):
            print(f'Starting loading spectrums with block size {self.block_size} in the following spectrum index {len(all_spectrums)} and line number {current_line_number}')

            spectrums, current_line_number = LoadData.get_all_spectrums_nist(file=file, 
            ...
            print(f'Saving spectrums with block size {self.block_size} in the following spectrum index {len(all_spectrums)} and updated line number {current_line_number}')
