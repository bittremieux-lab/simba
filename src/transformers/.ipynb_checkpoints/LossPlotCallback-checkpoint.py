...
class LossPlotCallback(pl.Callback):
    ...
    def _update_plot(self):
        reshaped_tr = np.array(self.train_loss_list).reshape(-1, self.batch_per_epoch_tr)
        reshaped_val = np.array(self.val_loss_list).reshape(-1, self.batch_per_epoch_val)
