...
class EmbedderFingerprint(Embedder):
    ...
    def forward(self, batch):
        ...
        emb0, _ = self.spectrum_encoder(
            mz_array=batch["mz_0"].float(), intensity_array=batch["intensity_0"].float()
        )
        emb1, _ = self.spectrum_encoder(
            mz_array=batch["mz_1"].float(), intensity_array=batch["intensity_1"].float()
        )
    ...
    def step(self, batch, batch_idx, threshold=0.5):
        ...
        target = (
            target[:, 0 : int(Config.D_MODEL)]
            - target[:, Config.D_MODEL : Config.D_MODEL * 2]
        )
        # apply weight loss
        ...
        loss = self.regression_loss(
            spec.float(), target.view(-1, Config.D_MODEL).float()
        ).float()
